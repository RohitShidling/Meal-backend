...
const getMyProfile = catchAsync(async (req, res, next) => {
  ...
  const professionalResult = await db.query(
    `
      SELECT id, client_id, name, company_name, corporate_location_id, city, state, lunch_time, created_at, updated_at
  ...
  const teacherResult = await db.query(
    `
      SELECT id, client_id, name, school_college_name, city, state, location, status, created_at, updated_at
